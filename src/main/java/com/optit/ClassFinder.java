...
public class ClassFinder implements Runnable {
    ...
    public boolean parseArguments(String[] args) {
        ...
        if (null == args || args.length == 0 || args.length == 1) {
            ...
            // Set defaults
            parameters.setProperty(com.optit.Parameters.matchCase, "false");
            parameters.setProperty(com.optit.Parameters.recursiveSearch, "false");
            parameters.setProperty(com.optit.Parameters.verbose, "false");
            parameters.setProperty(Parameters.partial, "true");

            for (int i = 0; i < args.length; i++) {
                if (Parameters.directory.equals(args[i])) {
                    parameters.setProperty(Parameters.directory, args[++i]);
                } else if (Parameters.classname.equals(args[i])) {
                    parameters.setProperty(Parameters.classname, args[++i]);
                } else if (Parameters.matchCase.equals(args[i])) {
                    parameters.setProperty(Parameters.matchCase, "true");
                } else if (Parameters.partial.equals(args[i])) {
                    parameters.setProperty(Parameters.partial, "true");
                } else if (Parameters.recursiveSearch.equals(args[i])) {
                    parameters.setProperty(Parameters.recursiveSearch, "true");
                } else if (Parameters.verbose.equals(args[i])) {
                    parameters.setProperty(Parameters.verbose, "true");
                } else if ("-help".equals(args[i]) || "--help".equals(args[i]) || "-?".equals(args[i]) || "--?".equals(args[i])) {
                    printHelp();
                    System.exit(0);
    ...
    public void printHelp() {
        ...
        logger.log("[-p]			Partial match (contains classname");
    ...
    public void findClass() {
        logger.setVerbose(parameters.getProperty(com.optit.Parameters.verbose).equals("true"));
        boolean matchCase = (parameters.getProperty(com.optit.Parameters.matchCase).equals("true"));
        boolean partial = (parameters.getProperty(Parameters.partial).equals("true"));

        // Class name contains some package qualifiers
        boolean containsPackageQualifier = (parameters.getProperty(com.optit.Parameters.classname).indexOf(".") != -1);
        // Change "." in package names to slashes (e.g. "org.apache.commons" -> "org/apache/commons")
        String classname = parameters.getProperty(com.optit.Parameters.classname).replaceAll("\\.", "/");
        ...
        buildFileList(new File(parameters.getProperty(com.optit.Parameters.directory)), parameters.getProperty(com.optit.Parameters.recursiveSearch).equals("true"));
        ...
        while (fileIterator.hasNext()) {
            ...
            if (fullFileName.endsWith(".class")) {
                ...
                if ((containsPackageQualifier && fullFileName.endsWith(classname + ".class"))
                        || (!containsPackageQualifier && fileName.equals(classname + ".class")) || (partial && fileName.contains(classname))) {
            ...
            // Direct java source file
                ...
                if ((containsPackageQualifier && fullFileName.endsWith(classname + ".java"))
                        ||
                        (!containsPackageQualifier && fileName.equals(classname + ".java")) || (partial && fileName.contains(classname))) {
                    logger.log(file.getName(), file.getAbsolutePath());
                }
            }
                ...
                try {
                    ...
                    while (entries.hasMoreElements()) {
                        ...
                        if (containsPackageQualifier) {
                            if (entryName.endsWith(classname + ".class") || entryName.endsWith(classname + ".java")|| (partial && entryName.contains(classname))) {
                        ...
                        // No package qualified, just Class Name
                            ...
                            if (entryName.endsWith("/" + classname + ".class") || entryName.endsWith("/" + classname + ".java")
                                    ||
                                    entryName.equals(classname + ".class") || entryName.equals(classname + ".java") || (partial && entryName.contains(classname))) {
                                logger.log(entry.getName(), file.getAbsolutePath());
                            }
                        }
                    }
    ...
    public void buildFileList(File directory, boolean recursive) {
        ...
        else if (!directory.isDirectory() && new com.optit.SearchableFileFilter().accept(directory)) {
            files.add(directory);
        } else {
            File[] dirs = directory.listFiles(new SearchableFileFilter());
            try {
                for (File file : dirs) {
                ...
                logger.logErr("directory= \"" + directory + "\"");
                e.printStackTrace();
            }
        }
    }
